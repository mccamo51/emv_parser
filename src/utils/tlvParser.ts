...
import { ParsedTLV, RequestType, TLVTag, ValidationResult } from "../types/tlv";
import { TLVValidator } from "./tlvValidator";
...
export class TLVParser {
  ...
  static parseTLV(
    ...
    try {
      ...
      if (requestType && requestType !== 'unknown') {
        validation = TLVValidator.validateTags(tags, requestType, isField48);
      }
  ...
  private static parseNextTag(
    ...
    position: number, 
    isField48: boolean
  ): { tag: TLVTag | null; nextPosition: number } {
    ...
    try {
      ...
      const tagDef = isField48 
        ? FIELD48_TAGS[tagResult.tag] 
        : EMV_TAGS[tagResult.tag];
   ...
   */
  private static parseTag(data: string, position: number): { tag: string; nextPosition: number } {
    if (position >= data.length) {
   ...
   */
  private static parseLength(data: string, position: number): { length: number; nextPosition: number } {
    if (position >= data.length) {
   ...
   */
  private static parseValue(hexValue: string, tagDef?: any): any {
    if (!tagDef || !hexValue) {
      return hexValue;
    }
   ...
   */
  private static parseAmount(hex: string): { sign: string; amount: string } {
    if (hex.length === 0) {
  ...
  private static getTagName(tag: string): string {
    const emvTag = EMV_TAGS[tag];
    const field48Tag = FIELD48_TAGS[tag];
